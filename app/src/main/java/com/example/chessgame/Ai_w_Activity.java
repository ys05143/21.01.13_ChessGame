...
//        if (game_ended(number) == true) end_game(1);
    ...
    public int RookMax(int[] node,int spot,int depth,int[] best_node) {
        ...
        for (int a = spot + 8; a < 64; a = a + 8) {//하
            if(in_board(a)) {
                if (!(node[a] >= 11 && node[a] <= 17)) {
                    ...
                    if (ret_val >= best_val) {
                        best_val = ret_val; //max(best_val,res_val)
                        temp_node = Node.clone(); //best_val이 갱신되었을 때의 move저장
                    }
                    // GenerateMove Cancel
                    Node = node.clone();
                }
        ...
        // return
        if(best_val!=-INF)
        for(int i=0 ; i<64 ; i++) best_node[i] = temp_node[i] ;  //temp_node에 저장해둔 것을 best_node로 변경 -> MaxMove의 ret_node가 변경됨.(call by ref)
